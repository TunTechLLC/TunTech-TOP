...
3. `scan_folder()` finds unprocessed `.txt` files in the engagement's folders
...
| Filename | Prompt |
|---|---|
| `Doc_Financial*.txt` | `FINANCIAL_EXTRACTION_PROMPT` |
| `Doc_Portfolio*.txt` | `PORTFOLIO_EXTRACTION_PROMPT` |
| `Doc_SOW*.txt` | `SOW_EXTRACTION_PROMPT` |
| `Doc_StatusReport*.txt` | `STATUS_EXTRACTION_PROMPT` |
| `Doc_Resource*.txt` | `RESOURCE_EXTRACTION_PROMPT` |
| `Doc_Delivery*.txt` | `DELIVERY_DOCUMENT_EXTRACTION_PROMPT` |
| `Doc_ClientFeedback*.txt` | `SIGNAL_EXTRACTION_PROMPT` (generic) |
| `Doc_Other*.txt` | `SIGNAL_EXTRACTION_PROMPT` (generic) |

Routing is implemented in `get_file_type()` in `api/services/document_processor.py`.
Stem matching is case-insensitive substring — `Doc_StatusReport_Q1.txt` matches `status`.
...
- Multi-format file ingestion (Excel, Word, PDF, PowerPoint) — currently `.txt` only
