...
export const api = {
  ...
  qaCoverage: {
    list:           (id)            => fetch(`${BASE}/engagements/${id}/qa-coverage`).then(handle),
    run:            (id)            => fetch(`${BASE}/engagements/${id}/qa-coverage/run`,
                                        { method: 'POST' }).then(handle),
    update:         (id, qid, data) => fetch(`${BASE}/engagements/${id}/qa-coverage/${qid}`,
                                        patch(data)).then(handle),
    confirmTier1:   (id)            => fetch(`${BASE}/engagements/${id}/qa-coverage/confirm-tier-1`,
                                        { method: 'POST' }).then(handle),
  },
  qaCoherence: {
    list:           (id)            => fetch(`${BASE}/engagements/${id}/qa-coherence`).then(handle),
    run:            (id)            => fetch(`${BASE}/engagements/${id}/qa-coherence/run`,
                                        { method: 'POST' }).then(handle),
    update:         (id, qid, data) => fetch(`${BASE}/engagements/${id}/qa-coherence/${qid}`,
                                        patch(data)).then(handle),
    confirmTier1:   (id)            => fetch(`${BASE}/engagements/${id}/qa-coherence/confirm-tier-1`,
                                        { method: 'POST' }).then(handle),
  },
  qaEditorial: {
    list:           (id)            => fetch(`${BASE}/engagements/${id}/qa-editorial`).then(handle),
    run:            (id)            => fetch(`${BASE}/engagements/${id}/qa-editorial/run`,
                                        { method: 'POST' }).then(handle),
    update:         (id, qid, data) => fetch(`${BASE}/engagements/${id}/qa-editorial/${qid}`,
                                        patch(data)).then(handle),
